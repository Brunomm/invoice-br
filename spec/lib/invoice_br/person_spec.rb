...
describe InvoiceBr::Person do
  let(:person) { InvoiceBr::Person.new }

  describe 'Attributes' do
    subject { described_class }
    it { is_expected.to have_attribute(:address,                      AddressType.optional.default( InvoiceBr::Address.new ) ) }
    ...
    it { is_expected.to have_attribute(:tax_code,                     InvoiceBr::Person::TaxCodeEnum.optional.default(nil) ) }
    it { is_expected.to have_attribute(:state_registration_indicator, InvoiceBr::Person::StateRegistrationIndicatorEnum.optional.default(nil) ) }
  ...
  describe '#simple_national?' do
    context 'returns true when' do
      it 'tax_code is 1' do
        person = InvoiceBr::Person.new(tax_code: '1')
        expect( person.simple_national? ).to eq(true)
        person = person.new(tax_code: 1)
        expect( person.simple_national? ).to eq(true)
      end
      it 'tax_code is 2' do
        person =  InvoiceBr::Person.new(tax_code: '2')
        expect( person.simple_national? ).to eq(true)
    ...
    context 'returns false when' do
      it 'tax_code is 3' do
        person = InvoiceBr::Person.new(tax_code: '3')
